...
describe('Migração card_invoice → cartão + faturas + pagamentos (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const [vault, box, cat] of [
      ...
      await runSql(`INSERT INTO vault (id, token, created_at) VALUES ('${vault}', 't-${vault}', now())`);
    ...
    const invoice = (id: string, vault: string, box: string, amount: number, paid: Date) =>
    ...
    const tx = (
      ...
      type: 'income' | 'expense',
      date: Date,
      extra: { invoiceId?: string; purchaseDate?: Date; categoryId?: string } = {},
    ) =>
    ...
    await tx('p-mercado', V, 'box-1', 1000, 'expense', d(9, 10), { invoiceId: 'inv-a', purchaseDate: d(8, 12), categoryId: 'cat-1' });
    await tx('p-posto', V, 'box-1', 1050, 'expense', d(9, 10), { invoiceId: 'inv-a', purchaseDate: d(8, 20), categoryId: 'cat-1' });
    await tx('rest-a', V, 'box-1', 1150, 'expense', d(9, 10), { invoiceId: 'inv-a' });
    ...
    await tx('rest-b', V, 'box-1', 500, 'expense', d(10, 10), { invoiceId: 'inv-b' });
    // Uma transação comum não é tocada.
    await tx('normal', V, 'box-1', 42, 'expense', d(8, 5), { categoryId: 'cat-1' });

    // Vault 2: sem extrato, pago 300 em 05/09, compra de 200 e estorno de 50
    // ligados, resto 150.
    await invoice('inv-c', V2, 'box-2', 300, d(9, 5));
    await tx('p-compra', V2, 'box-2', 200, 'expense', d(9, 5), { invoiceId: 'inv-c', purchaseDate: d(8, 20), categoryId: 'cat-2' });
    await tx('p-estorno', V2, 'box-2', 50, 'income', d(9, 5), { invoiceId: 'inv-c', purchaseDate: d(8, 25) });
    await tx('rest-c', V2, 'box-2', 150, 'expense', d(9, 5), { invoiceId: 'inv-c' });
  ...
  it('cria cartões, faturas e pagamentos (o pagamento guarda o id da fatura antiga)', async () => {
    ...
    expect(cards[1]).toMatchObject({ vault_id: V2, account_key: null, name: 'Cartão' });

    // Datas lidas como texto: o pg cru interpreta `timestamp` no fuso local.
    const payments = (await runSql(`SELECT p.*, c.card_id, to_char(c.period_start, 'YYYY-MM-DD') AS period_start,
      to_char(c.closing_date, 'YYYY-MM-DD') AS closing_date, to_char(c.due_date, 'YYYY-MM-DD') AS due_date
      FROM card_payment p JOIN card_cycle c ON c.id = p.invoice_id ORDER BY p.id`)).rows;
    expect(payments.map((p) => p.id)).toEqual(['inv-a', 'inv-b', 'inv-c']);
    const a = payments[0];
    expect(a).toMatchObject({ amount: 3200, imported: true, import_entry_id: 'entry-pay', card_id: cards[0].id });
    ...
    const batch = (await runSql(`SELECT invoice_id FROM import_batch WHERE id = 'batch-card'`)).rows[0];
    expect(batch.invoice_id).toBe(a.invoice_id);
    const tables = (await runSql(`SELECT to_regclass('card_invoice') AS t`)).rows[0];
    expect(tables.t).toBeNull();
  });

  it('as compras voltam à data da compra como compras de cartão; o resto antigo sai', async () => {
    const rows = (await runSql(`SELECT id, to_char(date, 'YYYY-MM-DD') AS date, invoice_role, purchase_date FROM "transaction" WHERE id IN ('p-mercado','p-estorno','rest-a','rest-b','normal') ORDER BY id`)).rows;
    const byId = Object.fromEntries(rows.map((r) => [r.id, r]));
    expect(byId['p-mercado']).toMatchObject({ invoice_role: 'purchase', purchase_date: null });
  ...
  it('mantém o total de cada mês igual ao modelo antigo e nas agregações do plano', async () => {
    ...
    const parts = [...vault.transactions.values()].filter((t) => t.isInvoicePart);
    expect(parts.map((t) => [t.sourceTransactionId, t.amount, t.date.toISOString().slice(0, 10)]).sort()).toEqual([
      ['p-mercado', 1000, '2026-09-10'],
      ['p-posto', 1050, '2026-09-10'],
    ]);
    const remainders = [...vault.transactions.values()].filter((t) => t.isInvoiceRemainder);
    ...
    expect(sept.filter((t) => t.type === 'expense').reduce((s, t) => s + t.amount, 0)).toBe(3200);
    ...
    const v2rows = [...v2.transactions.values()].filter((t) => t.isInvoiceDerived);
  ...
  it('rodar as migrações de novo não faz nada', async () => {
    await migrate(db, { migrationsFolder: MIGRATIONS });
    const count = (await runSql(`SELECT count(*)::int AS n FROM card_payment`)).rows[0];
    expect(count.n).toBe(3);
    const n = (await db.execute(sql`SELECT count(*)::int AS n FROM card`)).rows[0];
    expect(n.n).toBe(2);
  });
});
